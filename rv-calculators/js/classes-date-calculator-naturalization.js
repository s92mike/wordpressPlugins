(function( $ ) {
 ...
 class ProgressBar {
    ...
    start = () => {
      ...
      this.mainDiv.addClass(__progressBar).append(barDiv).delay(500).promise().done(this.tick);
    }
    tick = () => {
      const { complete } = this;
      $(`#` + __divID + ` .` + __progressBar + ` .` + __bar + ` .` + __filler).animate({ width: complete + `%` }, {
        duration: 1500,
        complete: () => {
  ...
  class ProcessSelector extends ProgressBar {
    ...
    displayProcess = () => {
      switch(this.currentProcess) {
        ...
        case 0:
          ...
          this.commonMainDivProgressBarFirst(questionairSelector.mainDiv.slideUp(`fast`));
          break;
        default:
          // this.start();
    ...
    commonMainDivSlide = function ( id ) {
      mainDiv.children().slideUp(`slow`, () => {
        ...
        this.commonHeightScript(questionairSelector.mainDiv.slideUp(`fast`), this.commonRemoveMainStyleSlide, arg);
      });
    }
    commonDisplayReset = () => {
      ___valid = null;
      ...
      ___truMarriage = null;
      this.currentProcess = 0;
      $(`#` + __divID).children().fadeIn(`slow`, function() { mainDiv.removeAttr(`style`); });
    ...
    commonRemoveMainStyleSlide = ( arg = false ) => {
      if (arg) {
        $(`#` + __divID + ` .` + arg.id).click(() => {
          questionairSelector.clickFunc({ type: arg.id, value: $(this).data(`answer`) });
          if (___valid) {
            this.displayProcess();
            this.currentProcess++;
          } else  {
      ...
      $(`#` + __divID).children().slideDown(`slow`, function() { mainDiv.removeAttr(`style`); });
    }
    commonHeightScript = ( commonMainDiv=null, doneFunc=()=>{ return; }, arg=false ) => {
      mainDiv.css(`height`, mainDiv.height() + `px`).html(``).append(commonMainDiv).delay(250).promise().done(doneFunc(arg));
    }
  }
  ...
  class QuestionaireBlock extends ProcessStepsLayout {
    ...
    greenCardIssue = () => {
      ...
      const dateContainerDiv = this.containerDiv({ classContainer: __dateContainer, elementBox: contentElement });
    ...
    twoButtonQuestion = ({ id, step }) => {
      ...
      const ButtonElementYes = this.greenButtonElement({ id, value: __processDataList[step].buttonLabel[0] });
      const ButtonElementNo = this.greenButtonElement({ id, value: __processDataList[step].buttonLabel[1] });
      const questionaireContainerDiv = this.containerDiv({ classContainer: __buttonContainer, elementBox: ButtonElementYes });
    ...
    now = (type) => {
      switch (type) {
        case __valid:
          ...
          const ButtonElement = this.greenButtonElement({ id: __startOver, value: `Start Over` });
          const btnContainer = $(`<div/>`).addClass(`buttons`).append(ButtonElement)
          container.append(__messageDetailList.valid.message + ` ` + __messageDetailList.eligibilityMessage + ` `)
            .append(bElement)
            .append(btnContainer);

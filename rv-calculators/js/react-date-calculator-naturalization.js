...
const __sequence = [__progress, __process, __loader, __summary];
const __defaultMain = {
  seq: 0,
  data: {
    date: "",
...
const __numberInText = ["zero", "one", "two", "three", "four", "five"];
  ...
  render() {
    ...
    return (
      <button
        ...
        className={"button conditional" + className}
      >
        {title}
      </button>
    );
    ...
    return (
      <div class="button-container">
        <button
          ...
          onClick={updateButton.bind(this, true)}
        >
          {first}
        </button>
        <button
          class={"button conditional" + myClassName}
          onClick={updateButton.bind(this, false)}
        >
          {second}
        </button>
      </div>
  ...
  render() {
    ...
    return (
      <div className="summary">
        <h4>
          Green Card issued: <b>{data.date}</b>
        </h4>
        <h4>
          Green Card obtained through marriage to a US citizen:{" "}
          <b>{yesOrNoText(data.married)}</b>
        </h4>
        <h4 className="valid">
          Green Card validity: <b>{yesOrNoText(data.valid)}</b>
        </h4>
        <h4 className="success-react">
          The soonest you can apply for citizenship is: <b>{natSched}</b>
        </h4>
        <span>
          (90 days before your {yearsToCitizenship} years permanent residency;
          Date format: YYYY/MM/DD)
        </span>
        {data.kind ? (
          <p>
            <b>Disclaimer: </b>You need a 10 Year Green Card (Removal of
            Condition) to apply for Naturalization Citizenship (Call us at
            800-872-1458 for help)
          </p>
        ) : (
          ""
        )}
...
class DisplayComponent extends PureComponent {
  state = {
    message: {
      validity: {
        content: (
          <h4>
            If your green card is no longer valid, you are not eligible to apply
            for citizenship<b>(Call us at 800-872-1458 for help)</b>
          </h4>
        ),
      ...
      married2YGC: {
        content: (
          <h4>
            If you don't have 10 Years Green Card(Removal of Conditions), you
            are not eligible to apply for citizenship{" "}
            <b>(Call us at 800-872-1458 for help)</b>
          </h4>
        ),
        divClassName: "info",
      },
  ...
  displaySummary = (arg) => {
    ...
    const natSched = calculateNaturalization({
      date: arg.date,
      yearsToCitizenship,
    });
    return (
      <Summary
        data={arg}
        natSched={natSched}
        yearsToCitizenship={__numberInText[yearsToCitizenship]}
      />
    );
  };
  ...
  render() {
    ...
    return (
      <div className={divClassName}>
        <div className="message-react">{content}</div>
...
class ProcessStep extends PureComponent {
    ...
    this.setState((prevState) => {
      ...
      const stepsData = [
        ...
        },
        {
          title: (
            <h4>
              Did you obtain your green card through marriage to US a citizen?
            </h4>
          ),
          component: (
            <TwoButtons
  ...
  updateStateCurrent = (arg = { marriage: false, val: false, type: false }) => {
    ...
    if (
      current < maxSteps &&
      (!(arg.marriage && arg.val === false) || (arg.type && arg.val === false))
    ) {
      this.setState((prevState) => {
